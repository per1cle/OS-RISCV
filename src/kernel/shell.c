...
// Helper pentru comparare string
...
// Helper pentru conversie string la int
...
// Comanda uptime - afișează tick counter
static void cmd_uptime(void) {
    ...
    // Calculează secunde (assuming ~100 ticks/sec based on TIMER_INTERVAL)
...
// Comanda mem - afișează statistici memorie
...
static void cmd_exec(const char *arg) {
    ...
    kernel_putstring("\n");  // Newline după exec pentru claritate
}

// Procesare comandă
static void process_command(char *cmd) {
    // Elimină spații de la început
    while (*cmd == ' ') cmd++;
    ...
    // Găsește sfârșitul primului cuvânt (comanda)
    char *arg = cmd;
    while (*arg && *arg != ' ') arg++;
    
    // Separă comanda de argumente
    if (*arg) {
        *arg = '\0';  // Termină comanda
        arg++;
        while (*arg == ' ') arg++;  // Sari peste spații
    }
...
void shell_run(void) {
    ...
    while (1) {
        ...
        // Citește comandă (cu polling non-blocking pentru a permite scheduler-ul)
        cmd_pos = 0;
        while (1) {
            int c = kernel_getchar_nonblock();
            
            if (c == -1) {
                // Nu este input disponibil, continuă polling
                // Adaugă o mică pauză pentru a nu consuma 100% CPU
                for (volatile int i = 0; i < 1000; i++);
                continue;
            }
            
            // Enter - procesează comanda
            if (c == '\r' || c == '\n') {
                kernel_putchar('\n');
            ...
            // Backspace
                ...
                if (cmd_pos > 0) {
                    cmd_pos--;
                    kernel_putstring("\b \b");  // Șterge caracterul de pe ecran
                }
            }
            // Caracter normal
            else if (c >= 32 && c < 127 && cmd_pos < CMD_BUFFER_SIZE - 1) {
                cmd_buffer[cmd_pos++] = (char)c;
                kernel_putchar((char)c);  // Echo
            }
        }
    }
}

// State pentru shell polling
...
// Polling non-blocking pentru shell (apelat din scheduler idle)
void shell_poll(void) {
    ...
    if (c == '\r' || c == '\n') {
        ...
        if (shell_cmd_pos > 0) {  // Nu procesa comenzi goale
