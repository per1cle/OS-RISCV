...
int create_process(void *entry_point, void *stack_top) {
    for(int i = 1; i < PROC_MAX; i++) {
        if(proc_table[i].state == UNUSED) {
            ...
            // Use wrapper to enable interrupts before jumping to entry_point
            extern void entry_point_wrapper(void);
            proc_table[i].context.ra = (long)entry_point_wrapper;
            proc_table[i].context.s0 = (long)entry_point;
...
void preempt_from_handler(void *stack_frame){
    if(proc_actual > 0) {
        ...
        // Load CSRs from stack frame (saved in start.S)
        proc_table[proc_actual].trap_frame.mepc = sf[124/4];
        proc_table[proc_actual].trap_frame.mstatus = sf[4/4];
